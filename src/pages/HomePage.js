...
import { Modal, Form, Input, message, Select, Table } from "antd";
import { UnorderedListOutlined, AreaChartOutlined, EditOutlined, DeleteOutlined } from "@ant-design/icons";
import Layout from "../components/Layout/Layout"; // Ensure the correct path
  ...
  useEffect(()=>{
    const getAllTransaction=async()=>{
      try{
        ...
        setLoading(true)
        const res= await axios.post('/api/v1/transections/get-transection',{userid:user._id,filter})
        setLoading(false);
        setAllTransection(res.data);
        console.log(res.data);
      }catch(error){
        console.log(error);
        message.error('Issue in Transection');
...
setEditable(null);
      ...
      setLoading(false)
      message.error("Pls fill the fields "); 
    }
  ...
  return (
   ...
   {loading && <Spinner/>}
      <div className="filters">
        <div>
          <h6>Filter</h6>
          <Select value={filter} onChange={(values)=> setFilter(values)}></Select>
          <Select>
      ...
      >
        <Form layout="Vertical" onFinish={handleSubmit} initialValues={editable}>
          ...
          <div className="d-flex justify-content-end"><button type="submit" className="btn btn-primary">{" "}Save</button>
